...
class AudienceDeepeningAgent(BaseDecisionAgent):
    ...
    def execute(
        ...
        cluster_insights = self._generate_cluster_insights(normalised, breakthrough, brand_name)
            ...
            out.append(
                {
                    ...
                    "value_proposition": p.get("value_proposition", p.get("message", "")),
                }
            )
        if not out:
            out = [
        ...
        breakthroughs = [
            {
                ...
                "estimated_size": "high",
                "urgency": "high",
                "gap": ("No comparison or technical-deep-dive content in existing library."),
            },
            {
                "name": "economic_buyer",
                "description": (
    ...
    def _generate_cluster_insights(
        ...
        insights = [
            ...
            (
                f"Across {len(existing)} existing persona(s),"
                f" {unique_pains} unique pain point(s) documented."
            ),
            (
        ...
        if breakthroughs:
            insight_gaps = [b["gap"] for b in breakthroughs if b.get("gap")]
            if insight_gaps:
                insights.append("Content gaps for breakthrough clusters: " + " ".join(insight_gaps))
