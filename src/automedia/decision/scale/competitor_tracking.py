...
class CompetitorTrackingAgent(BaseDecisionAgent):
    ...
    @staticmethod
    def _normalise_competitors(raw: list[Any], brand: str, market: str) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def _derive_counter_positioning(competitors: list[dict[str, Any]], brand: str) -> list[str]:
        ...
        for comp in competitors:
            ...
            for s in shifts:
                if "expanding" in s.lower():
                    ...
                    plays.append(
                        f"Position {brand} as the privacy-first, transparent alternative to {name}."
                    )
        ...
        if not unique:
            unique.append(
                f"{brand} occupies a differentiated position — no direct counter-play required."
            )

        return unique

    @staticmethod
    def _find_blue_ocean(competitors: list[dict[str, Any]], brand: str, market: str) -> list[str]:
        ...
        crowded = {w for w in crowded_areas if any(w in s.lower() for s in all_shifts)}
