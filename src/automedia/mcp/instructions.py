...
def generate_instructions(tool_registry: dict[str, dict[str, Any]]) -> str:
    ...
    for cat_name, tool_names in _TOOL_CATEGORIES:
        ...
        label = (
            f"━━━ {cat_name.upper()} "
            f"({len(available)} tool{'s' if len(available) != 1 else ''}) "
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        )
    ...
    lines.extend(
        [
            ...
            "",
            "The LIVE human-review path is the H0 gate: when a pipeline hits",
            "H0 it pauses awaiting review and the MCP server resumes or halts",
            "it via the review_decision tool (same-process only):",
            "  review_decision(project_id, gate_name, action='approve')",
            "    → resumes the pipeline",
            "  review_decision(project_id, gate_name, action='reject', reason=...)",
            "    → halts the pipeline (stop-failure)",
            "  review_decision(..., show_diff=True)",
            "    → attaches a unified diff from the latest .automedia/gate_diffs/",
            "      record (diff_unavailable: true when no record exists)",
            "  Same-process constraint: review_decision reaches only pipelines",
            "  started by THIS MCP server (run_pipeline daemon threads). A",
            "  CLI-started pipeline runs in a separate process and CANNOT be",
            "  approved/rejected via MCP — decide it in its own terminal",
            "  (the call returns a structured error immediately, no deadlock).",
            "",
            ...
            "'awaiting_hitl'), then call review_decision for H0 pauses (the",
            "live path) or approve_gate/reject_gate for director-mode pauses.",
            "",
