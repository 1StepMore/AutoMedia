...
def tenant_create(
    ...
    if is_json:
        output_json({"status": "ok", "workspace_id": ws["workspace_id"], "name": ws["name"]})
    else:
        typer.secho(
...
def tenant_invite(
    ...
    if role not in ROLES:
        msg = f"Invalid role '{role}'. Valid roles: {', '.join(ROLES)}"
        if is_json:
    ...
    if not ok:
        ...
        raise typer.Exit(1)
    if is_json:
        output_json({"status": "ok", "workspace": workspace, "email": email, "role": role})
    else:
        typer.secho(
...
def tenant_list() -> None:
    ...
    if is_json:
        output_json({"status": "ok", "items": workspaces, "count": len(workspaces)})
        return
    if not workspaces:
    ...
    for ws in workspaces:
        typer.echo(f"{ws['workspace_id']:<14} {ws['name']:<20} {ws['member_count']:<8}")
...
def tenant_members(
    ...
    if is_json:
        output_json({"status": "ok", "items": members, "count": len(members)})
        return
    if not members:
...
def tenant_delete(
    ...
    if not ok:
        ...
        raise typer.Exit(1)
    if is_json:
        output_json({"status": "ok", "workspace": workspace})
    else:
...
def tenant_audit_log() -> None:
    ...
    if is_json:
        output_json({"status": "ok", "items": entries, "count": len(entries)})
        return
    if not entries:
...
def rbac_grant(
    ...
    if not ok:
        msg = f"Failed to grant role. Check workspace '{workspace}' and user '{user}'."
        if is_json:
            output_error_json(msg)
        else:
            typer.secho(msg, err=True, fg=typer.colors.RED)
...
def rbac_revoke(
    ...
    if not ok:
        msg = f"Failed to revoke. Check workspace '{workspace}' and user '{user}'."
        if is_json:
            output_error_json(msg)
        else:
            typer.secho(msg, err=True, fg=typer.colors.RED)
        raise typer.Exit(1)
    if is_json:
        output_json({"status": "ok", "workspace": workspace, "user": user})
    else:
...
def rbac_check(
    ...
    if is_json:
        output_json({"status": "ok", "role": role, "action": action, "allowed": allowed})
        if not allowed:
            raise typer.Exit(1)
        return
    if allowed:
