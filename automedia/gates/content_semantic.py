...
def _check_keyword_coverage(
    ...
    if coverage >= _MIN_COVERAGE:
        return {
            "name": name,
            "passed": True,
            "detail": f"coverage {coverage:.1%} >= {_MIN_COVERAGE:.0%} ({len(matched)}/{len(source_set)})",
        }
    return {
        "name": name,
        "passed": False,
...
def _check_source_alignment(source_texts: list[str]) -> dict[str, Any]:
    ...
    if present >= 2:
        return {"name": name, "passed": True, "detail": f"{present} of {len(source_texts)} sources present"}
    return {"name": name, "passed": False, "detail": f"only {present} of {len(source_texts)} sources present (need >= 2)"}
...
class V3ContentSemantic(BaseGate):
    ...
    def execute(self, gate_context: dict[str, Any]) -> dict[str, Any]:
        ...
        check_fns: list[tuple[str, Any]] = [
            ("keyword_coverage", lambda: _check_keyword_coverage(source_keywords, content_keywords)),
            ("source_alignment", lambda: _check_source_alignment(source_texts)),
            ("no_hallucination", lambda: _check_no_hallucination(source_keywords, content_keywords)),
        ]
