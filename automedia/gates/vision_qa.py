...
def _check_mid_frame_valid(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if missing:
        return {"name": name, "passed": False, "detail": f"entries {missing} missing mid_frame_path"}
    return {"name": name, "passed": True, "detail": f"all {len(entries)} entries have mid_frame_path"}
...
def _check_end_silence_frame_valid(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if missing:
        return {"name": name, "passed": False, "detail": f"entries {missing} missing end_silence_frame_path"}
    return {"name": name, "passed": True, "detail": f"all {len(entries)} entries have end_silence_frame_path"}
...
def _check_red_line_6(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    name = "red_line_6"
    if not entries:
        return {"name": name, "passed": False, "detail": "no entries to verify (Red Line 6: full coverage required)"}
    unchecked = [i for i, e in enumerate(entries) if not e.get("checked", False)]
    if unchecked:
        return {"name": name, "passed": False, "detail": f"entries {unchecked} not checked — Red Line 6 requires full coverage"}
    return {"name": name, "passed": True, "detail": f"Red Line 6 satisfied: all {len(entries)} entries checked (full coverage)"}
