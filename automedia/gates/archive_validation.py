...
def _check_force_flag(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if force:
        return {"name": name, "passed": True, "detail": "--force flag is set, overriding archive status"}
    return {"name": name, "passed": True, "detail": "no --force flag (archive status must be 'published')"}
