...
_TEMPLATE_CONCLUSION_RES: list[re.Pattern[str]] = [
    re.compile(p) for p in _TEMPLATE_CONCLUSIONS
]
...
def _check_long_conjunctions(content: str) -> dict[str, Any]:
    ...
    if not found:
        return {"name": "long_conjunctions", "passed": True, "detail": "no long conjunction chains found"}
    return {
    ...
    for sent in sentences:
        for pat in _TEMPLATE_CONCLUSION_RES:
            ...
            if m:
                ...
                break
    if not found:
        return {"name": "template_conclusions", "passed": True, "detail": "no template conclusions found"}
    return {
...
def _check_overacademic_vocabulary(content: str) -> dict[str, Any]:
    ...
    if not matches:
        return {"name": "overacademic_vocabulary", "passed": True, "detail": "no over-academic words found"}
    found = list(set(m.lower() for m in matches))
    return {
...
def _check_absolute_assertions(content: str) -> dict[str, Any]:
    ...
    if not matches:
        return {"name": "absolute_assertions", "passed": True, "detail": "no absolute assertions found"}
...
def _check_repetitive_structures(content: str) -> dict[str, Any]:
    ...
    if len(sentences) < 3:
        return {"name": "repetitive_structures", "passed": True, "detail": "too few sentences to check"}
    ...
    return {"name": "repetitive_structures", "passed": True, "detail": "no repetitive structures found"}
...
class G1Humanizer(BaseGate):
    ...
    def execute(self, gate_context: dict[str, Any]) -> dict[str, Any]:
        ...
        return _build_result(checks, modified_content=modified_content)
