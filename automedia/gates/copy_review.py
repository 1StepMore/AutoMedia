...
def _check_tone(content: str, brand_profile: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for conflicting in conflicting_tones:
        ...
        if found:
            issues.append(
                f"conflicting '{conflicting}' tone markers found: {', '.join(found[:3])}"
            )
    ...
    if target_keywords:
        ...
        if not present:
            issues.append(
                f"expected '{target_tone}' tone but no matching keywords found"
            )
...
def _check_evidence(content: str) -> dict[str, Any]:
    ...
    if evidence:
        return {
            "name": name,
            "passed": True,
            "detail": f"{len(claims)} claim(s) found, {len(evidence)} evidence indicator(s) present",
        }
...
def _check_specificity(content: str) -> dict[str, Any]:
    ...
    if concrete_count >= abstract_count:
        return {
            "name": name,
            "passed": True,
            "detail": f"{abstract_count} abstract word(s) balanced by {concrete_count} concrete indicator(s)",
        }
    ...
    return {
        "name": name,
        "passed": False,
        "detail": f"{abstract_count} abstract word(s) but only {concrete_count} concrete example(s): {', '.join(found[:5])}",
    }
