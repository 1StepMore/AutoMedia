...
def _check_duration_valid(
    ...
    if expected_min <= actual_duration <= expected_max:
        return {
            "name": name,
            "passed": True,
            "detail": f"duration {actual_duration:.1f}s in [{expected_min:.1f}, {expected_max:.1f}]",
        }
    return {
        "name": name,
        "passed": False,
        "detail": f"duration {actual_duration:.1f}s outside [{expected_min:.1f}, {expected_max:.1f}]",
    }
...
class V7SixStepHard(BaseGate):
    ...
    def execute(self, gate_context: dict[str, Any]) -> dict[str, Any]:
        ...
        check_fns: list[tuple[str, Any]] = [
            ...
            ("duration_valid", lambda: _check_duration_valid(actual_duration, expected_duration_min, expected_duration_max)),
        ]
