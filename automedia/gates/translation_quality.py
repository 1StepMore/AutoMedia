...
def _get_translation_md(translation_result: Any) -> str:
...
class L4TranslationQuality(BaseGate):
    ...
    def run(self, context: dict[str, Any]) -> GateResult:
        ...
        **failures** occurred (warnings from garbled-text are non-blocking).
        ...
        frontmatter_ok, frontmatter_data, frontmatter_error = _parse_frontmatter(
            translated_md
        )
        ...
        if frontmatter_ok and frontmatter_data is not None:
            ...
            if not lang_ok:
                ...
                if not source_ok:
                    parts.append(
                        f"source_lang: frontmatter={actual_source!r}, "
                        f"expected={expected_source!r}"
                    )
                if not target_ok:
                    parts.append(
                        f"target_lang: frontmatter={actual_target!r}, "
