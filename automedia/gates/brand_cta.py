...
def _check_brand_identity(
    ...
    identity_declared_ok = any(
        expected in declared_identity for expected in expected_identities
    ) if declared_identity else None

    # Check content for identity phrases
    content_has_identity = any(
        phrase in content for phrase in _BRAND_IDENTITY_PHRASES
    )
...
class G3BrandCTA(BaseGate):
    ...
    def execute(self, gate_context: dict[str, Any]) -> dict[str, Any]:
        ...
        check_fns: list[tuple[str, Any]] = [
            ...
            ("cta_direction_sync", lambda: _check_cta_direction_sync(content, video_script, brand_profile)),
