...
_FORBIDDEN_PATTERNS: list[dict[str, Any]] = [
    {
        ...
        "patterns": [
            r"公益", r"科普", r"慈善", r"charity", r"public\s*science",
            r"donation", r"fundraising", r"募捐",
        ],
    },
    {
        "check": "topic_not_gov_tool",
        "patterns": [
            r"政府", r"政策", r"政务", r"government", r"policy",
            r"administration", r"bureau", r"ministry",
        ],
    },
    {
        "check": "topic_not_investment",
        "patterns": [
            r"投资", r"invest", r"investment", r"venture",
            r"capital", r"equity", r"portfolio",
        ],
    },
    {
        "check": "topic_not_finance",
        "patterns": [
            r"金融", r"finance", r"financial", r"股票", r"stock",
            r"基金", r"fund", r"理财", r"wealth", r"banking",
        ],
    },
    {
    ...
    for pattern in patterns:
        if re.search(pattern, topic_lower):
            return {
                "name": check_name,
                "passed": False,
                "detail": f"topic matches forbidden category '{category_label}' (matched: /{pattern}/)",
            }
    return {
        "name": check_name,
        "passed": True,
...
def _check_topic_length_valid(topic: str) -> dict[str, Any]:
    ...
    if len(stripped) < 5:
        return {"name": name, "passed": False, "detail": f"topic too short ({len(stripped)} chars, min 5)"}
    if len(stripped) > 500:
        return {"name": name, "passed": False, "detail": f"topic too long ({len(stripped)} chars, max 500)"}
    return {
        "name": name,
        "passed": True,
