...
_GRAPH_PATH = Path(__file__).resolve().parent.parent.parent / "solution-wise" / "process" / "dependency-graph.yaml"
...
def get_nodes_for_mode(mode: str) -> list[dict[str, Any]]:
    ...
    return [
        n for n in _load_graph().get("nodes", [])
...
def get_required_nodes_for_mode(mode: str) -> set[int]:
    ...
    optional_ids = {n.get("node_id") for n in _load_graph().get("nodes", []) if n.get("optional")}
    all_ids = {n.get("node_id") for n in get_nodes_for_mode(mode)}
