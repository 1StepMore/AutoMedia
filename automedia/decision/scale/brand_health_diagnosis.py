...
class BrandHealthDiagnosisAgent(BaseDecisionAgent):
    ...
    def _score_awareness(brand: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if score is not None:
            return {"score": max(0, min(100, int(score))), "details": raw.get("details", "Provided by upstream.")}
    ...
    def _score_competitiveness(brand: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "score": score,
            "details": (
                f"{len(competitors)} competitor(s) tracked; "
                f"{len(usp)} USP(s) identified."
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if overall == "critical":
            recs.insert(0, "Immediate brand-strategy intervention required — engage executive steering.")
