...
def complete_node(
    node: str = typer.Option(
        ..., "--node", "-n", help="Node name (e.g. brand_questionnaire).",
    ),
    brand: str = typer.Option(
        ..., "--brand", "-b", help="Brand identifier.",
    ),
) -> None:
    ...
    if node_id is None:
        all_names = sorted(
            n.get("name", "?") for n in dependency.list_all_nodes()
        )
        typer.secho(
    ...
    if not (1 <= node_id <= 27):
        typer.secho(
            f"Invalid node ID {node_id} for {node!r} — expected 1-27.",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        typer.secho(
            f"Failed to read state file: {exc}",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1)
...
def approve_node(
    node: str = typer.Option(
        ..., "--node", "-n", help="Node name to approve.",
    ),
    by: str = typer.Option(
        ..., "--by", help="User identifier (who approved).",
    ),
) -> None:
...
def next_node(
    ...
    mode_nodes = [
        n for n in dependency.list_all_nodes()
        if n.get("mode") in (mode, "both")
    ]
    ...
    if _is_human:
        typer.secho(
            f"Next node {next_node_info['name']!r} requires human "
            f"intervention (HITL).",
...
def validate_artifact(
    ...
    if not isinstance(data, dict):
        typer.secho(
            "Artifact data must be a JSON object / dictionary.",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1)
...
def preflight_check(
    ...
    if not warnings:
        typer.secho(
            f"Preflight OK — all prerequisites for phase {next_phase} "
            f"({mode}) are complete.",
            fg=typer.colors.GREEN,
        )
        return
