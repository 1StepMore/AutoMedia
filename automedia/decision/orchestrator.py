...
class DecisionOrchestrator:
    ...
    def __init__(self, hitl_config: Any = None) -> None:
        ...
        self._brand_positioning = BrandPositioningAgent()
        self._market_research = MarketResearchAgent()
        self._audience_seg = AudienceSegmentationAgent()
        self._competitor_analysis = CompetitorAnalysisAgent()
        self._health_diagnosis = BrandHealthDiagnosisAgent()
        self._market_reval = MarketRevalidationAgent()
        self._audience_deepen = AudienceDeepeningAgent()
        self._competitor_track = CompetitorTrackingAgent()
        self._asset_audit = ContentAssetAuditAgent()
        self._product_opt = ProductOptimizationAgent()
        self._content_mkt = ContentMarketingAgent()
    ...
    def convert_to_pipeline_input(
        ...
        if strategy:
            ...
            if pillars and not topic:
                topic = pillars[0].get("name", "") if isinstance(pillars[0], dict) else str(pillars[0])
