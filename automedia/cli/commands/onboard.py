...
_STEPS = [
    "llm", "brand", "pipeline", "platforms",
    "hitl", "cron", "omni", "prompts", "all",
]
...
def _step_llm() -> None:
    ...
    api_key = typer.prompt("API key (leave blank to use env AUTOMEDIA_LLM_API_KEY)", default="", hide_input=True)
...
def _step_brand() -> None:
    ...
    tone = typer.prompt("Tone (professional/casual/humorous)", default=existing.get("tone", "professional"))
    cta_raw = typer.prompt("CTA principles (comma-separated)", default=", ".join(existing.get("cta_principles", ["Include clear CTA", "Use action verbs"])))
    blocked_raw = typer.prompt("Blocked words (comma-separated)", default=", ".join(existing.get("blocked_words", [])))
...
def _default_tts(lang: str) -> str:
    voices = {"zh": "zh-CN-YunxiNeural", "en": "en-US-JennyNeural", "ja": "ja-JP-NanamiNeural", "ko": "ko-KR-SunHiNeural"}
...
def _step_pipeline() -> None:
    ...
    text_enabled = typer.prompt("Enable text track", default=pipeline.get("text", {}).get("enabled", True))
    image_enabled = typer.prompt("Enable image track", default=pipeline.get("image", {}).get("enabled", True))
    video_enabled = typer.prompt("Enable video track", default=pipeline.get("video", {}).get("enabled", True))
    audio_enabled = typer.prompt("Enable audio track", default=pipeline.get("audio", {}).get("enabled", True))
...
def _step_platforms() -> None:
    ...
    defaults = {
        "wechat": True, "zhihu": True, "xiaohongshu": True,
        "youtube": False, "tiktok": False, "twitter": False,
    }
...
def _step_omni() -> None:
    ...
    enable = typer.prompt("Enable Omni document processing (requires OPP/OL/ORF packages)", default=False)
    if not enable:
...
def _step_prompts() -> None:
    ...
    copy = typer.prompt("  Copy default prompts to overrides directory for customization?", default=True)
    if not copy:
...
@app.command()
def start(
    step: str = typer.Option("all", "--step", "-s", help="Run a specific step: llm, brand, pipeline, platforms, hitl, cron, omni, prompts, all"),
) -> None:
    """Run the onboarding wizard for one or all steps."""
    steps = _STEPS if step == "all" else [step]

    if step not in _STEPS:
        typer.secho(f"Unknown step: {step!r}. Choose from: {', '.join(_STEPS)}", fg=typer.colors.RED, err=True)
...
def status() -> None:
    ...
    checks = [
        ...
        ("Platform config", _USER_CFG_DIR / "config.yaml" if (_USER_CFG_DIR / "config.yaml").exists() and "platforms" in (_USER_CFG_DIR / "config.yaml").read_text() else None),
...
def list() -> None:
    ...
    config = load_config()
    typer.echo(f"LLM provider: {config.get('llm', {}).get('text_generation', {}).get('provider', 'not set')}")
    typer.echo(f"LLM model: {config.get('llm', {}).get('text_generation', {}).get('model', 'not set')}")
    ...
    if brand_path.exists():
        ...
        typer.echo(f"Languages: {list(brand.get('languages', {}).keys()) or ['not configured']}")
