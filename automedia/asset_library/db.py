...
class AssetDatabase:
    ...
    def add_asset(self, doc: AssetDoc) -> str:
        ...
        stmt = f"INSERT INTO assets ({columns}) VALUES ({placeholders})"
        try:
    ...
    def _update_by_id(self, doc: AssetDoc) -> None:
        ...
        sets = ", ".join(f"{k} = :{k}" for k in row.keys() if k != "doc_id")
        stmt = f"UPDATE assets SET {sets} WHERE doc_id = :doc_id"
    ...
    def get_asset(self, doc_id: str) -> dict[str, Any] | None:
        ...
        cur = self.conn.execute(
            "SELECT * FROM assets WHERE doc_id = ?", (doc_id,)
        )
        row = cur.fetchone()
        if row is None:
            return None
    ...
    def delete_asset(self, doc_id: str) -> None:
        ...
        self.conn.execute(
            "DELETE FROM assets WHERE doc_id = ?", (doc_id,)
        )
        self.conn.commit()
