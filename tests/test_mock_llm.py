...
class TestMockLlmResponseDirect:
    ...
    def test_returns_provided_data(self) -> None:
        ...
        with mock_llm_response(expected):
...
class TestMockLlmFailure:
    ...
    def test_raises_on_direct_call(self) -> None:
        ...
        with mock_llm_failure(), pytest.raises(TimeoutError, match="LLM API timeout"):
            llm_complete_structured_safe(
                "prompt",
                response_format=G0CheckResult,
            )
...
class TestAssertLlmCalled:
    ...
    def test_passes_when_called_once(self) -> None:
        ...
        data = G0CheckResult(passed=True, issues=[])
        with mock_llm_response(data) as mock, assert_llm_called(mock, expected_calls=1):
            llm_complete_structured_safe(
                "prompt",
                response_format=G0CheckResult,
            )
    ...
    def test_fails_when_not_called(self) -> None:
        ...
        data = G0CheckResult(passed=True, issues=[])
        with (
            mock_llm_response(data) as mock,
            pytest.raises(AssertionError, match="Expected 1 LLM call"),
            assert_llm_called(mock, expected_calls=1),
        ):
            pass  # no LLM call
    ...
    def test_passes_with_custom_count(self) -> None:
        ...
        data = G0CheckResult(passed=True, issues=[])
        with mock_llm_response(data) as mock, assert_llm_called(mock, expected_calls=3):
            for _ in range(3):
                llm_complete_structured_safe(
                    "prompt",
                    response_format=G0CheckResult,
    ...
    def test_fails_on_wrong_count(self) -> None:
        ...
        with (
            mock_llm_response(data) as mock,
            pytest.raises(AssertionError, match="Expected 2 LLM call"),
            assert_llm_called(mock, expected_calls=2),
        ):
            llm_complete_structured_safe(
                "prompt",
                response_format=G0CheckResult,
...
class TestLlmMockFixture:
    ...
    def test_failure_helper_works(self, llm_mock: dict[str, Any]) -> None:
        ...
        with llm_mock["failure"](), pytest.raises(TimeoutError):
            llm_complete_structured_safe(
                "prompt",
                response_format=G0CheckResult,
            )
    ...
    def test_assert_called_helper_works(self, llm_mock: dict[str, Any]) -> None:
        ...
        data = G0CheckResult(passed=True, issues=[])
        with (
            llm_mock["response"](data) as mock,
            llm_mock["assert_called"](mock, expected_calls=1),
        ):
            llm_complete_structured_safe(
                "prompt",
                response_format=G0CheckResult,
