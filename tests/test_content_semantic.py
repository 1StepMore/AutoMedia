...
from automedia.gates.content_semantic import V3ContentSemantic, _build_result, _CHECK_NAMES
...
def _make_context(
    *,
    ...
    ctx: dict[str, Any] = {
        "source_keywords": source_keywords if source_keywords is not None else ["ai", "ml", "data", "model", "train"],
        "content_keywords": content_keywords if content_keywords is not None else ["ai", "ml", "data", "model", "train", "extra"],
...
class TestV3MockDriven:
    ...
    def test_keyword_coverage_failure(self) -> None:
        result = V3ContentSemantic().execute(_make_context(mock_results=_fail_check("keyword_coverage", "low coverage")))
        assert result["passed"] is False

    def test_source_alignment_failure(self) -> None:
        result = V3ContentSemantic().execute(_make_context(mock_results=_fail_check("source_alignment")))
        assert result["passed"] is False

    def test_no_hallucination_failure(self) -> None:
        result = V3ContentSemantic().execute(_make_context(mock_results=_fail_check("no_hallucination")))
        assert result["passed"] is False
...
class TestV3RealLogic:
    def test_full_coverage_passes(self) -> None:
        result = V3ContentSemantic().execute(_make_context(
